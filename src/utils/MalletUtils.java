...
public class MalletUtils {
	...
	public static InstanceList binarizeLabelsNewest(InstanceList instances, String label) {
		...
		//iterate through each instance and change the target based on the original value.
		for (int i = 0; i < iListCopy.size(); i++) {
			Instance inst = (Instance) iListCopy.get(i).clone();

			inst.unLock();
			if (inst.getLabeling().toString().equals(label)) {  
				inst.setTarget(((LabelAlphabet) newIList.getTargetAlphabet()).lookupLabel(label));
			} else {
				inst.setTarget(((LabelAlphabet) newIList.getTargetAlphabet()).lookupLabel("not_"+label));
			}
			newIList.add(inst);
		}	
		return newIList;
	...
	public static InstanceList binarizeLabelsNew(InstanceList instances, String label) {
		
		String alterLabel;
		int idx = instances.getTargetAlphabet().lookupIndex(label);
		
		if(idx!=0) {
			alterLabel = (String) instances.getTargetAlphabet().lookupObject(0);
		}else {
			alterLabel = (String) instances.getTargetAlphabet().lookupObject(1);
		}
		...
		for (int i = 0; i < iListCopy.size(); i++) {
		   ...
		   inst.unLock();
		   if (inst.getLabeling().toString().equals(label)) {  
		       inst.setTarget(((LabelAlphabet) inst.getTargetAlphabet()).lookupLabel(label));
		   } else {
		       inst.setTarget(((LabelAlphabet) inst.getTargetAlphabet()).lookupLabel(alterLabel));
		   }
		   iListCopy.set(i, inst);
		}	
	...
	public static void main(String[] args) {
		...
		InstanceList instances = MalletUtils.getInstancesFromLibSVMFile("/Users/shashans/Desktop/zoo_features.libsvm");
		System.out.println(instances.size());
		System.out.println(instances.get(0).getLabeling());
		FeatureVector fv = (FeatureVector) instances.get(1).getData();
		for(int i=0;i<fv.getIndices().length;i++) {
			System.out.print(instances.getDataAlphabet().lookupObject(fv.getIndices()[i])+":"+fv.getValues()[i]+" ");
		}
		System.out.println();
