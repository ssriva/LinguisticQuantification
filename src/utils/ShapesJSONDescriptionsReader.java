...
public class ShapesJSONDescriptionsReader {

	private static int numValidConcepts = 0, numValidStatements = 0;

	public static void main(String[] args) {		

		String readDirectory = "data/shapes/mturk/";
		String writeDirectory = "data/shapes/descriptions/";
	...
	public static void processJSONShapesDescriptions(String fileName, String fileString, String writeDirectory) {
		...
		try {
			...
			List<String> statements = IntStream.range(0, 10).mapToObj(i -> (String) jsonObject.get("statement_"+i)).filter(obj -> obj!=null).collect(Collectors.toList());
	...
	private static void writeDescriptionsToFile(String fileString, String writeDirectory, List<String> statements) {
		try {
			...
			statements.stream().forEach(s -> writer.println(s));
			writer.close();
